...
// We worked with and got help from Januel and his team
...
/* Sets the organism's value based on its location
*/
...
void Board::printBoard()
 {
    ...
    for (int r = 0; r < totalRows; r++)
    {
	 ...
	 // Draws the organism and left/right borders
...
void Board::updateBoard() {
	...
	for (int r = 1; r < activeRows; r++)
	{
	   ...
	   {
	     /* During each cycle of play, an organism is born into an empty location which has exactly three neighboring locations containing organism
	      */
		...
		_board[r][c] = DYING;
	     ...
	     // All organisms with more than three neighboring organisms die of overcrowing
	...
	for (int r = 0; r < totalRows; r++)
	{
	   for (int c = 0; c < totalCols; c++)
	   {
	     // Changes the new organism to LIVING
	     if (_board[r][c] == GESTATING)
	     {
		_board[r][c] == LIVING;
	     }
	     // Changes the dying organism to NONE
